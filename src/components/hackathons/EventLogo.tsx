...
const EventLogo: React.FC<EventLogoProps> = ({ source, size, style, isDark }) => {
    ...
    const bgColor = sourceStr === 'hackerearth'
        ? (isDark ? '#2A3F5F' : '#3176B9')
        : (isDark ? '#563D7C' : '#6C4AA0');
        ...
        >
            {sourceStr === 'hackerearth' && (
                <>
                    ...
                    <Text style={[styles.logoText, { fontSize: size * 0.12, color: textColor }]}>HackerEarth</Text>
                </>
            )}

            {sourceStr === 'devfolio' && (
                <>
                    <MaterialCommunityIcons name={"rocket-launch"} size={iconSize} color={textColor} />
                    <Text style={[styles.logoText, { fontSize: size * 0.12, color: textColor }]}>Devfolio</Text>
                </>
            )}
            ...
            {sourceStr !== 'hackerearth' && sourceStr !== 'devfolio' && (
                <>
                    <Ionicons name={DEFAULT_GENERIC_ICON} size={iconSize} color={textColor} />
                    <Text style={[styles.logoText, { fontSize: size * 0.12, color: textColor }]}>Event</Text>
                </>
            )}
        </View>
    );
