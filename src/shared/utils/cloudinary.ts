...
const extractCloudinaryPublicId = (url: string): string | null => {
  try {
    ...
    logger.error('Error extracting public_id from URL', error, 'cloudinary');
    return null;
  }
};
...
const deleteOldProfilePhoto = async (oldPhotoUrl: string) => {
  try {
    ...
    if (!publicId) {
      logger.warn(
        'Could not extract public_id from old profile photo URL',
        undefined,
        'cloudinary',
      );
      return;
    ...
    logger.debug(
      'Deleting old profile photo with public_id',
      publicId,
      'cloudinary',
    );
    ...
    if (response.ok) {
      logger.info('Successfully deleted old profile photo', data, 'cloudinary');
    } else {
      logger.error(
    ...
    logger.error('Error deleting old profile photo', error, 'cloudinary');
  }
};
